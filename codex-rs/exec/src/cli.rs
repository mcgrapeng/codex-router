...
#[command(
    version,
    override_usage = "codexrouter exec [OPTIONS] [PROMPT]\n       codexrouter exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Codex Router outside a Git repository.
    ...
    /// Do not load `$CODEXROUTER_HOME/config.toml`; auth still uses the configured Codex Router home.
...
struct ResumeArgsRaw {
    ...
    // so "codexrouter resume --last <prompt>" treats the positional as a prompt, not a session id.
