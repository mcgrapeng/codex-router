...
`codex-exec-server` is the library backing `coder exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`codex-utils-pty`.

It provides:

- a CLI entrypoint: `coder exec-server`
...
`coder exec-server` command builds these paths from the native CLI arg0 dispatch
state.
