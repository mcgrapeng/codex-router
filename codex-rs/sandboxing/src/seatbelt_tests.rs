...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.codexrouter".to_string(),
    ...
    assert!(
        args.iter()
            .any(|arg| arg.starts_with("-DWRITABLE_ROOT_0_EXCLUDED_")
                && arg.ends_with("/workspace/.codexrouter")),
        "expected proactive .codexrouter carveout for cwd root: {args:#?}",
    );
}
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // top-level .git and .codexrouter directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
        vulnerable_root_canonical,
        dot_git_canonical,
        dot_codexrouter_canonical,
        empty_root,
    ...
    // write to .codexrouter/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected cwd .codexrouter carveout in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_0")
            && policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_1"),
        "expected explicit writable root .git/.codexrouter carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_codexrouter_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .codexrouter/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_codexrouter_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .codexrouter is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_codexrouter_creation_with_exact_and_descendant_carveouts()
 {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_codexrouter.to_string_lossy().as_ref(),
    ...
    assert!(
        ...
        "expected exact .codexrouter carveout in policy:\n{policy_text}"
    );
    assert!(
        policy_text.contains("(require-not (subpath (param \"WRITABLE_ROOT_0_EXCLUDED_1\")))"),
        "expected descendant .codexrouter carveout in policy:\n{policy_text}"
    );
}
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.codexrouter` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .codexrouter), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
struct PopulatedTmp {
    /// Path containing a .git and .codexrouter subfolder.
    ...
    /// ran `git commit` themselves, or modified .codexrouter/config.toml to
    ...
    dot_codexrouter_canonical: PathBuf,

    /// Path without .git or .codexrouter subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".codexrouter")).expect("create .codexrouter");
    fs::write(
        vulnerable_root.join(".codexrouter").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .codexrouter/config.toml");
    ...
    let dot_codexrouter_canonical = vulnerable_root_canonical.join(".codexrouter");
