...
"#,
    ...
    write_file(
        &repo_root.join(".codexrouter/config.toml"),
        r#"[[skills.config]]
...
async fn load_plugins_ignores_project_config_files() {
    ...
    write_file(
        &project_root.join(".codexrouter/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                project_config_folder: AbsolutePathBuf::try_from(project_root.join(".codexrouter"))
                    .unwrap(),
            },
