...
async fn ignore_user_config_keeps_empty_user_layer() -> std::io::Result<()> {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when CODEXROUTER_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when CODEXROUTER_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codexrouter")).await?;
    tokio::fs::create_dir_all(project_root.join(".codexrouter")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".codexrouter").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codexrouter").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".codexrouter").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".codexrouter").as_path()
    );
...
"#;
    ...
    .await?;
    tokio::fs::write(
        project_root.join(".codexrouter").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".codexrouter").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(".codexrouter");
    tokio::fs::create_dir_all(&codex_home).await?;
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".codexrouter");
    let nested_dot_codex = nested.join(".codexrouter");
...
-> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let dot_codex = project_root.join(".codexrouter");
    let codex_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".codexrouter")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".codexrouter").join(CONFIG_TOML_FILE), "foo =").await?;
