...
//!
//! The history is stored at `~/.codexrouter/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.codexrouter`.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.codexrouter/history.jsonl` and ensure the parent directory exists.
