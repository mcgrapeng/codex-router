...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_project_config_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            project_config_folder: project_project_config_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let root_rules = project_root.join(".codexrouter").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".codexrouter").join(RULES_DIR_NAME);
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".codexrouter").join(RULES_DIR_NAME);
