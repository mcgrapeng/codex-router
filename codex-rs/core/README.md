...
pointer file), the resolved `gitdir:` target, and `.codexrouter` read-only.
...
Expects the binary containing `codex-core` to run the equivalent of `coder sandbox linux` (legacy alias: `coder debug landlock`) when `arg0` is `codex-linux-sandbox`. See the `codex-arg0` crate for details.
