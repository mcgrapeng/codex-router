...
/// This test writes to a fallback credentials file in CODEXROUTER_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire CODEXROUTER_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _codex_home_guard = EnvVarGuard::set("CODEXROUTER_HOME", temp_home.path().as_os_str());
