...
//!
//! If the keyring is not available or fails, we fall back to CODEXROUTER_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Codex Router MCP Credentials";
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create CODEXROUTER_HOME temp dir");
            unsafe {
                std::env::set_var("CODEXROUTER_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("CODEXROUTER_HOME");
            }
        }
    }
