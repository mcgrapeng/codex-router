...
Set a stable path to the skill CLI (default `CODEXROUTER_HOME` is `~/.codexrouter`):

```
export CODEXROUTER_HOME="${CODEXROUTER_HOME:-$HOME/.codexrouter}"
export IMAGE_GEN="$CODEXROUTER_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$CODEXROUTER_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
