...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    let dot_codexrouter = tmpdir.path().join(".codexrouter");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codexrouter).expect("create .codexrouter");

    let git_target = dot_git.join("config");
    let codex_target = dot_codexrouter.join("config.toml");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".codexrouter write should be denied under bubblewrap",
    );
...
async fn sandbox_blocks_codex_symlink_replacement_attack() {
    ...
    let dot_codexrouter = tmpdir.path().join(".codexrouter");
    symlink(&decoy, &dot_codexrouter).expect("create .codexrouter symlink");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".codexrouter symlink replacement should be denied",
    );
    assert_ne!(codex_output.exit_code, 0);
}
