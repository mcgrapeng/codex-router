...
- Server binary: `coder mcp-server` (or `codex-mcp-server`)
...
```bash
coder mcp-server | your_mcp_client
```
...
Use the separate `coder mcp` subcommand to manage configured MCP server launchers in `config.toml`.
