...
pub struct Arg0DispatchPaths {
    ...
    /// instead of the real Codex Router CLI.
...
///
/// 1.  Load `.env` values from `CODEXROUTER_HOME/.env` before creating any threads.
...
/// Load env vars from CODEXROUTER_HOME/.env.
///
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    // Use a CODEXROUTER_HOME-scoped temp root to avoid cluttering the top-level directory.
