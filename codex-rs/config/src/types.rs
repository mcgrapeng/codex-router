...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in CODEXROUTER_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in CODEXROUTER_HOME.
    Auto,
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// CODEXROUTER_HOME/.credentials.json
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$CODEXROUTER_HOME/themes` for custom themes.
    #[serde(default)]
