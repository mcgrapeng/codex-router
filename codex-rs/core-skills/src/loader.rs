    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$CODEXROUTER_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$CODEXROUTER_HOME/skills/.system` and are a
