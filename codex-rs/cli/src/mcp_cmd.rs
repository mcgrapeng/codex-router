...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "codexrouter mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `codexrouter mcp add my-tool -- my-command`.");
        return Ok(());
    }
