...
//! interactive-session layers. Direct `coder login` intentionally does less: it preserves the
...
/// Installs a small file-backed tracing layer for direct `coder login` flows.
///
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `coder login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `coder login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | coder login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `coder login` working in environments where device-code may be disabled/feature-gated.
