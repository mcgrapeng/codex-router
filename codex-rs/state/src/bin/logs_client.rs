...
struct Args {
    /// Path to CODEXROUTER_HOME. Defaults to $CODEXROUTER_HOME or ~/.codexrouter.
    #[arg(long, env = "CODEXROUTER_HOME")]
...
fn default_codex_home() -> PathBuf {
    if let Some(codexrouter_home) = std::env::var_os("CODEXROUTER_HOME") {
        return PathBuf::from(codexrouter_home);
    }
    if let Some(home) = home_dir() {
        return home.join(".codexrouter");
    }
    PathBuf::from(".codexrouter")
}
